from numpy import frompyfunc, arange, array, sum 
from scipy.optimize import fminbound
from sympy.mpmath import hyperu, mp, sqrt, pi, atan, hyp2f1, gamma, erfinv, rgamma, exp, power, quad, findroot
...
def B_01(Non, Noff, alpha):
	...
	'''
	#gamma, delta, c1/c2
	gam = (1.+2.*Noff)*alpha**(0.5+Non+Noff)*gamma(0.5+Non+Noff)
	delta = 2.*(1.+alpha)**(Non+Noff)*gamma(1+Non+Noff)*hyp2f1(0.5+Noff,1.+Non+Noff,1.5+Noff,(-1./alpha))
...
def C_lambda(lambda_s, Non, Noff, alpha, maxdeg=4,err=True,meth='gauss-legendre'):
	'''
...
if __name__ == "__main__":
	...
	#plot the pdf
	x_range = arange(0,2.0*float(l_99_buf),0.1)
